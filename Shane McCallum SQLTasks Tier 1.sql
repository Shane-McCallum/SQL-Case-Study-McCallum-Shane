...
SELECT facid, membercost, monthlymaintenance, name
FROM Facilities
...
CASE WHEN monthlymaintenance > 100.0
THEN 'Expensive'
ELSE 'cheap'
END AS 'Expensive/Cheap'
FROM Facilities

/* Q6: You'd like to get the first and last name of the last member(s)
who signed up. Try not to use the LIMIT clause for your solution. */

SELECT CONCAT( firstname, ' ', surname ) AS 'Member Name', joindate
FROM Members
WHERE joindate = (
SELECT MAX( joindate )
FROM Members )
...
SELECT name AS 'Facility Name:', SUM(
CASE WHEN memid =0
THEN guestcost * slots
ELSE membercost * slots
END ) AS Revenue
